...
def extract():
    ...
    pizzas = pd.read_csv("data_files/pizzas.csv", encoding="latin-1")
    pizza_types = pd.read_csv("data_files/pizza_types.csv", encoding="latin-1")
    orders = pd.read_csv("data_files/orders.csv", encoding="latin-1")
    order_details = pd.read_csv("data_files/order_details.csv", encoding="latin-1")
...
def transform(pizzas, pizza_types, orders, order_details):
    ...
    orders = orders.drop(columns=["time", "pizza_id", "order_id", "order_details_id", "name", "category"])
    ...
    orders['date'] = pd.to_datetime(orders['date'], format="%d/%m/%Y") - pd.to_timedelta(7, unit='d')
